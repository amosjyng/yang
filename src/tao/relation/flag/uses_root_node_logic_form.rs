...
impl From<UsesRootNodeLogic> for Flag {
    fn from(this: UsesRootNodeLogic) -> Flag {
        Flag::from(this.base)
    }
}
...
mod tests {
    ...
    fn check_type_attributes() {
        initialize_kb();
        assert_eq!(UsesRootNodeLogic::archetype().added_attributes(), vec![]);
        #[rustfmt::skip]
        assert_eq!(UsesRootNodeLogic::archetype().attributes(), vec![Owner::archetype()]);
    }
