...
const YANG_DEP_VERSION: &str = "0.0.5";
...
fn main() {
    ...
    if let Ok(_) = env::var("DOCS_RS") {
        println!(
            "docs.rs build detected. docs.rs does not allow for network access, so we will \
            just continue with the rest of the build since all the files should already be \
            generated."
        );
        exit(0);
    }
